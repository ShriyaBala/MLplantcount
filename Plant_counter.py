...
ESCAPE_KEY = 27
RESIZABLE_WINDOW = cv2.WINDOW_NORMAL
...
def createTrackbars():
    ...
    cv2.createTrackbar(MIN_HUE_TB_LABEL, TB_WIN_LABEL,
                       MIN_GREEN_HUE, MAX_HUE_VALUE, nothing)
    cv2.createTrackbar(MIN_SAT_TB_LABEL, TB_WIN_LABEL,
                       MIN_GREEN_SAT, MAX_SAT_VALUE, nothing)
    cv2.createTrackbar(MIN_VAL_TB_LABEL, TB_WIN_LABEL,
                       MIN_GREEN_VAL, MAX_VAL_VALUE, nothing)
    cv2.createTrackbar(MAX_HUE_TB_LABEL, TB_WIN_LABEL,
                       MAX_GREEN_HUE, MAX_HUE_VALUE, nothing)
    cv2.createTrackbar(MAX_SAT_TB_LABEL, TB_WIN_LABEL,
                       MAX_GREEN_SAT, MAX_SAT_VALUE, nothing)
    cv2.createTrackbar(MAX_VAL_TB_LABEL, TB_WIN_LABEL,
                       MAX_GREEN_VAL, MAX_VAL_VALUE, nothing)
    cv2.createTrackbar(KERNEL_SIZE_TB_LABEL, TB_WIN_LABEL,
                       DEFAULT_KERNEL_SIZE, MAX_KERNEL_SIZE, nothing)
    cv2.createTrackbar(ERODE_ITERATIONS_TB_LABEL, TB_WIN_LABEL,
                       DEFAULT_ERODE_ITERATIONS, MAX_ERODE_ITERATIONS, nothing)
    cv2.createTrackbar(DILATE_ITERATIONS_TB_LABEL, TB_WIN_LABEL,
                       DEFAULT_DILATE_ITERATIONS, MAX_DILATE_ITERATIONS, nothing)
...
def updateJson(srcPath, dstPath, data):
    ...
    operations.append(
        "erode " + data["erodeKernel"] + " x" + data["erodeIterations"])
    operations.append(
        "dilate " + data["dilateKernel"] + " x" + data["dilateIterations"])

    jsonData["images"][dstImgname] = {}
    jsonData["images"][dstImgname]["date"] = time.strftime(
        "%Y-%m-%d", time.gmtime())
...
def main(srcPath, dstPath):
    ...
    src = cv2.imread(dstPath) 
    # ressize the image to new dim
    ...
    for i in range(contoursLen):
        ...
        if perimeter > PERIMETER_LIMIT:
            plantsNumber += 1
            val = (i + 1) * colorStep
            cv2.drawContours(src, [contours[i]], -1,
                             (val, val, val), LINE_WIDTH)
            print("(" + str(val) + "," + str(val) + "," +
                  str(val) + ") : " + str(perimeter))
    ...
    if dstPath != None:
        ...
        data["erodeKernel"] = "(" + str(kernelSize) + \
            "," + str(kernelSize) + ")"
        data["erodeIterations"] = str(erodeIterations)
        data["dilateKernel"] = "(" + str(kernelSize) + \
            "," + str(kernelSize) + ")"
